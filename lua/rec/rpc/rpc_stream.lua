  ...
  return setmetatable({
    ...
    _session = mpack.Session({
      unpack = mpack.Unpacker({
        ext = {
          -- Buffer
          [0] = function(_c, s) return mpack.decode(s) end,
          -- Window
          [1] = function(_c, s) return mpack.decode(s) end,
          -- Tabpage
          [2] = function(_c, s) return mpack.decode(s) end,
        },
      }),
    }),
  }, RpcStream)
...
function RpcStream:read_start(on_request, on_notification, on_eof)
  self._stream:read_start(function(data)
    if not data then return on_eof() end
...
function RpcStream:read_stop() self._stream:read_stop() end

function RpcStream:close(signal, noblock) self._stream:close(signal, noblock) end
