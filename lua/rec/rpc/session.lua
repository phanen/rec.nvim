...
local uv = vim.uv
local RpcStream = require('rec.rpc.rpc_stream')
...
local coroutine_exec = function(func, ...)
  local args = { ... }
...
function Session:next_message(timeout)
  ...
  if self._is_running then error('Event loop already running') end

  if #self._pending_messages > 0 then return table.remove(self._pending_messages, 1) end
...
function Session:notify(method, ...) self._rpc_stream:write(method, { ... }) end
...
function Session:run(request_cb, notification_cb, setup_cb, timeout)
  ...
  local on_notification = function(method, args) coroutine_exec(notification_cb, method, args) end

  self._is_running = true

  if setup_cb then coroutine_exec(setup_cb) end
...
function Session:stop() uv.stop() end
...
function Session:_yielding_request(method, args)
  return coroutine.yield(function(co)
    self._rpc_stream:write(method, args, function(err, result) resume(co, err, result) end)
  end)
end
...
function Session:_run(request_cb, notification_cb, timeout)
  if type(timeout) == 'number' then
    self._prepare:start(function()
      self._timer:start(timeout, 0, function() uv.stop() end)
      self._prepare:stop()
    end)
  end
