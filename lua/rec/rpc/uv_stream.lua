...
function StdioStream:write(data) self._out:write(data) end
...
function StdioStream:read_stop() self._in:read_stop() end
...
function SocketStream.open(file)
  ...
  uv.pipe_connect(socket, file, function(err) self._stream_error = self._stream_error or err end)
  return self
end
...
function SocketStream.connect(host, port)
  ...
  uv.tcp_connect(
    ...
    host,
    port,
    function(err) self._stream_error = self._stream_error or err end
  )
  return self
end

function SocketStream:write(data)
  if self._stream_error then error(self._stream_error) end
...
function SocketStream:close() uv.close(self._socket) end
...
function ProcStream.spawn(argv, env, io_extra, on_exit)
  local self = setmetatable({
    ...
    output = function(self)
      if not self.collect_text then error('set collect_text=true') end
  ...
  self._proc, self._pid = uv.spawn(prog, {
    ...
    if self._on_exit then self._on_exit(self._closed) end
  end)
...
function ProcStream:write(data) self._child_stdin:write(data) end
...
function ProcStream:read_start(on_stdout, on_stderr)
  self._child_stdout:read_start(
    function(err, chunk) self:on_read('stdout', on_stdout, err, chunk) end
  )
  self._child_stderr:read_start(
    function(err, chunk) self:on_read('stderr', on_stderr, err, chunk) end
  )
end
...
function ProcStream:close(signal, noblock)
  ...
  if type(signal) == 'string' then self._proc:kill('sig' .. signal) end
  if not noblock then
